<?php
/* 
*	Класс для работы с устройствами из экосистемы xiaomi по протоколу miIO.
*
	...
	- поиск устройств (mdns-discovery)
...
class miIO {
	...
	public function __construct($ip = NULL, $token = NULL, $debug = false) {
		...
		if ($this->debug) {
			echo "Взаимодействие с устройством IP $this->ip" . PHP_EOL;
	...
	*/
	...
	public function Discover($ip = NULL) {
		...
		if ($ip != NULL) {
			//handshake
			if ($this->debug) echo PHP_EOL . 'Проверяем доступность устройства' . PHP_EOL;
			...
			socket_sendto($sock, $helloPacket, strlen($helloPacket), 0, $this->ip, MIIO_PORT);
